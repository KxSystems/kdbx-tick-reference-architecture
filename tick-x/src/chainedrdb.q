// tick-x/src/chainedrdb.q - Chained Realtime Database (Query Role)
//
...
//
// q tick-x/src/chainedrdb.q -p $CHAINED_RDB_PORT -tpPort $TICK_PORT \
...
// @desc Watermark file (hsym) — written by the writedown RDB at <idbDir>/.watermark, holding the
// single timespan "every row with time < W is persisted to the IDB". The chained RDB serves the
// `rdb` tier = the un-persisted tail only, so it sheds rows below W. That keeps `rdb` and `idb`
// disjoint, so a gateway `all` fan-out (rdb + idb + hdb) never double-counts intraday rows.
...
// @desc Drop rows older than `w` from every `g#sym` root table (in-memory shed, no disk I/O)
...
// @desc Push entry point — the TP relays the writedown RDB's post-flush watermark here so the
// shed happens promptly (and rides the same ordered TP connection as `upd`, so every to-be-shed
// row has already arrived). See `.u.broadcastWatermark` in u.q.
//
// @param w       {timespan}  Flush cutoff to shed below
.rdb.shedTo:{[w] .rdb.shedBelow w};

// @desc Re-derive the shed boundary from the watermark file and apply it. Used post-replay on
// (re)connect and as a periodic backstop in case a push broadcast was missed (e.g. a TP blip).
...
.rdb.connectTP:{[]
    ...
    if[null h; :0b];
    TP_H::h;
    // Register as a shed target so the TP relays the writedown RDB's watermark to us, then replay.
    .u.rep . h ({.u.regShed[]; ((.u.sub[;`] each .u.t); `.u `i`L)};::);
    // Replay rebuilds the whole day; shed everything already flushed to the IDB so the `rdb` tier
    // holds only the un-persisted tail (disjoint from `idb`).
    .rdb.applyWatermark[];
...
// @desc Shed backstop — re-applies the watermark on each housekeeping tick. Push (`.rdb.shedTo`)
// keeps the tiers disjoint in steady state; this guarantees eventual convergence if a broadcast
// was ever missed (e.g. the writedown RDB briefly lost its TP connection mid-flush).
.timer.funcs[`rdbShed]:{[] .rdb.applyWatermark[]};
