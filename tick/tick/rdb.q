/q tick/tick/rdb.q [host]:port[:usr:pwd] [host]:port[:usr:pwd]
/2008.09.09 .k ->.q
...
// TP and HDB endpoints. Ports come in bare; we prepend "::" for TCP loopback.
.u.x:("::",first CLI_ARGS[`tpPort];"::",first CLI_ARGS[`hdbPort]);

// EOD: save today's tables to HDB partition, reset RDB tables
.u.end:{
...
// Connect to TP, subscribe, and replay log
