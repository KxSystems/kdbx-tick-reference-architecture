...
- Ensure scripts under `scripts/tick/` are executable.
...
```bash
$ ./scripts/tick/startup.sh -e /path/to/.env
```
...
Live data publishing is driven by the timer interval and initialised automatically when the system starts. The interval can also be overridden at runtime using the `scripts/tick/fh-timer.sh` script.
...
All processes write structured logs to `PROCESS_LOG_DIR` in the format `<procName>_<datetime>.log`. The `scripts/tick/monitor.sh` watchdog script checks whether each process is alive and restarts any that have died.

```bash
# Check and restart any dead processes
$ ./scripts/tick/monitor.sh

# Run as a cron job every minute
* * * * * cd /path/to/kdbx-tick-reference-architecture && ./scripts/tick/monitor.sh -e .env >> app/proclogs/monitor-cron.log 2>&1
```
...
$ ./scripts/tick/restart.sh RDB_CHAIN_0 -m 1
```
...
- A `startup.log` file is created by `scripts/tick/startup.sh`.
...
`scripts/tick/fh-timer.sh` must be sourced to expose two functions for dynamic timer control. Both functions open and close an IPC connection inline, allowing interval adjustments at runtime without restarting the FH process.

```bash
source ./scripts/tick/fh-timer.sh
