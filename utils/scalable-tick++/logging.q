...
.log.initFile:{[proc;dt]
    // Safety: PROCESS_LOG_DIR must be set - otherwise we'd try to write at filesystem root
    if[0=count getenv[`PROCESS_LOG_DIR];
        -2 "ERROR: PROCESS_LOG_DIR env var not set. Source .env first, or set PROCESS_LOG_DIR before launching the process.";
        exit 1;
    ];
