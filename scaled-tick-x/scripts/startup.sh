...
# Start all processes for the Scaled Tick-X Reference Architecture.
# Run from the project root directory.
#
# Usage: ./scaled-tick-x/scripts/startup.sh [-e envFile] [-s secondaries] [-m chainedRdbs]
...
print_usage() {
  printf "Usage: ./scaled-tick-x/scripts/startup.sh [-e envFile] [-s secondaries] [-m chainedRdbs]\n"
...
if [ "$m_flag" -lt 1 ]; then
  echo "scaled-tick-x requires at least one chained RDB replica (-m >= 1):"
...
echo -e "Starting Scaled Tick-X Reference Architecture..."
echo -e "  .env:             [$e_flag]"
