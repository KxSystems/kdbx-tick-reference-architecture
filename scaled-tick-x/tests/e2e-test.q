...
//   source .env && q scaled-tick-x/tests/e2e-test.q -gwPort $GW_PORT -restPort $REST_PORT -tpPort $TICK_PORT -fhPort $FH_PORT -procName e2e
...
apiCmd:"q scaled-tick-x/tests/api-test.q -gwPort ",string[GW_PORT]," -procName api-test";
...
restCmd:"q scaled-tick-x/tests/rest-test.q -restPort ",string[REST_PORT]," 2>&1";
.log.info["Running: q scaled-tick-x/tests/rest-test.q -restPort ",string REST_PORT];
...
if[not null gwPidI;
    ...
    system "kill -9 ",gwPid;
    system "sleep 0.5";
    system "./scaled-tick-x/scripts/restart.sh GW -e .env -m 2";
    system "sleep 3";
