# Scaled Tick-X Reference Architecture

A deployable template for a scalable KDB-X Tick-X architecture with additional layers for custom configuration.
...
![scaled-tick-x architecture](../arch/scaled-tick-x.drawio.png)
...
```bash
$ ./scaled-tick-x/scripts/startup.sh
Starting Scaled Tick-X Reference Architecture...
  .env:             [.env]
  Secondaries:      [0]
  Chained RDBs:     [1]
...
```bash
$ ./scaled-tick-x/scripts/startup.sh -e /path/to/.env
```
...
- **-s**
  ...
  ```bash
  $ ./scaled-tick-x/scripts/startup.sh -s 4
  ```
...
- **-m**
  ...
  ```bash
  $ ./scaled-tick-x/scripts/startup.sh -m 2
  ```
...
```bash
$ ./scaled-tick-x/scripts/shutdown.sh
Killing processes:
  TP          [118666]
  IDB         [118667]
  RDB         [118668]
...
$ ./scaled-tick-x/scripts/restart.sh RDB_CHAIN_0 -m 1
...
2026.05.06D09:07:36.465107038 info PID[71505] HOST[hostname] TP started using command: q scaled-tick-x/tick.q ...
```
...
| CLI arg `-logLevel` | `q scaled-tick-x/rte.q ... -logLevel debug ...` | One process (takes precedence over env) |
...
$ ./scaled-tick-x/utils/rotate-logs.sh

# Keep only 3 days of proclogs, 14 days of tplogs
$ ./scaled-tick-x/utils/rotate-logs.sh --keep-days 3 --tp-keep-days 14
```
...
```bash
source ./scaled-tick-x/scripts/fh-timer.sh
...
```bash
./scaled-tick-x/scripts/startup.sh -m 2
source .env && q scaled-tick-x/tests/e2e-test.q -gwPort $GW_PORT -restPort $REST_PORT \
