// scaled-tick-x/src/tick.q - Tickerplant Process
//
// Globals used:
...
//
// q scaled-tick-x/src/tick.q -p $TICK_PORT -schemaDir $SCHEMA_DIR -tplogDir $TPLOG_DIR -procName TP

system"l scaled-tick-x/utils/main.q";

.log.info["Initializing tickerplant"];

// @desc Registry of RDB / RDB_CHAIN subscribers — used to drive leader/follower failover
// Each subscribing RDB upserts its row via the lambda sent from `.rdb.connectTP` (rdb.q).
// `.u.failoverRDB` and the `.z.pc` hook in u.q react when a tracked handle closes.
.u.RDB_CONNECTIONS:([handle:`int$()];procName:`$();alive:`boolean$();leader:`boolean$());

// @desc Promote the next alive RDB_CHAIN to leader if the leader handle just closed
// Called from the augmented `.z.pc` hook in u.q whenever the closing handle is one we track.
// Flips the new leader's MAIN_FLAG to 1b via IPC so it begins writing down at EOD.
//
// @param h       {int}       Handle of the RDB process that just disconnected
.u.failoverRDB:{[h]
